...
// Fetch generated by name
$gen_stmt = $pdo->prepare("SELECT full_name FROM users WHERE id = ?");
$gen_stmt->execute([$bill['generated_by']]);
$bill['generated_by_name'] = $gen_stmt->fetchColumn() ?: 'System';
...
function renderBillHTML($is_pdf = false) {
    ...
    <body>
        ...
        <div class="ui-chrome no-print">
            ...
            <?php
            $wa_patient = urlencode($bill['full_name'] ?? 'Patient');
            $wa_bill    = urlencode($bill['bill_number'] ?? '');
            $wa_amount  = urlencode('₹' . number_format((float)($bill['net_amount'] ?? 0), 2));
            $wa_phone   = preg_replace('/\D/', '', $bill['phone'] ?? '');
            $wa_msg     = urlencode("Dear {$bill['full_name']}, your bill ({$bill['bill_number']}) of ₹" . number_format((float)($bill['net_amount'] ?? 0), 2) . " has been generated at Sankhla Hospital. Thank you.");
            $wa_link    = "https://wa.me/{$wa_phone}?text={$wa_msg}";
            ?>
            <a href="<?php echo $wa_link; ?>" target="_blank" class="btn" style="background:#25D366; color:#fff;" title="Share via WhatsApp">
                <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" fill="currentColor" viewBox="0 0 16 16" style="vertical-align:middle; margin-right:3px;"><path d="M13.601 2.326A7.85 7.85 0 0 0 7.994 0C3.627 0 .068 3.558.064 7.926c0 1.399.366 2.76 1.057 3.965L0 16l4.204-1.102a7.9 7.9 0 0 0 3.79.965h.004c4.368 0 7.926-3.558 7.93-7.93A7.9 7.9 0 0 0 13.6 2.326zM7.994 14.521a6.6 6.6 0 0 1-3.356-.92l-.24-.144-2.494.654.666-2.433-.156-.251a6.56 6.56 0 0 1-1.007-3.505c0-3.626 2.957-6.584 6.591-6.584a6.56 6.56 0 0 1 4.66 1.931 6.56 6.56 0 0 1 1.928 4.66c-.004 3.639-2.961 6.592-6.592 6.592m3.615-4.934c-.197-.099-1.17-.578-1.353-.646-.182-.065-.315-.099-.445.099-.133.197-.513.646-.627.775-.114.133-.232.148-.43.05-.197-.1-.836-.308-1.592-.985-.59-.525-.985-1.175-1.103-1.372-.114-.198-.011-.304.088-.403.087-.088.197-.232.296-.346.1-.114.133-.198.198-.33.065-.134.034-.248-.015-.347-.05-.099-.445-1.076-.612-1.47-.16-.389-.323-.335-.445-.34-.114-.007-.247-.007-.38-.007a.73.73 0 0 0-.529.247c-.182.198-.691.677-.691 1.654s.71 1.916.81 2.049c.098.133 1.394 2.132 3.383 2.992.47.205.84.326 1.129.418.475.152.904.129 1.246.08.38-.058 1.171-.48 1.338-.943.164-.464.164-.86.114-.943-.049-.084-.182-.133-.38-.232"/></svg>
                WhatsApp
            </a>
        ...
        <div class="bill-container">
            ...
            <!-- Footer -->
            <div class="footer-sec" style="position: relative;">
                <?php echo $footer_text; ?>
                <div style="text-align: right; margin-top: 20px; font-weight: bold;">
                    Generated By: <?php echo htmlspecialchars($bill['generated_by_name'] ?? 'System'); ?>
                </div>
            </div>
        </div>
    </body>
